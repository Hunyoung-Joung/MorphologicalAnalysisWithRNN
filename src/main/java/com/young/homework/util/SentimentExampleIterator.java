package com.young.homework.util;
...
public class SentimentExampleIterator implements DataSetIterator {
    ...
    private DataSet nextDataSet(int num) throws IOException {
        ...
        INDArray features = Nd4j.create(new float[]{reviews.size(), vectorSize, maxLength}, 'f');
        INDArray labels = Nd4j.create(new float[]{reviews.size(), 2, maxLength}, 'f');    //Two labels: positive or negative
        //Because we are dealing with reviews of different lengths and only one output at the final time step: use padding arrays
        //Mask arrays contain 1 if data is present at that time step for that example, or 0 if data is just padding
        INDArray featuresMask = Nd4j.zeros(reviews.size(), maxLength, 'f');
        INDArray labelsMask = Nd4j.zeros(reviews.size(), maxLength, 'f');
    ...
    public INDArray loadFeaturesFromString(String reviewContents, int maxLength){
        ...
        INDArray features = Nd4j.create(vectorSize, outputLength);
